...
function AccountsContent() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'invoices' | 'reminders' | 'bank' | 'medical'>('dashboard');
  ...
  const paged = <T,>(key: string, rows: T[]) => {
    ...
    return { rows: sliceForPage<T>(rows, current, size), page: current, size };
  };
  const pagedInvoices = paged('invoices', invoices);
  const pagedClaims = paged('claims', claims);
  const pagedAuthorizations = paged('authorizations', authorizations);
  const pagedReminders = paged('reminders', reminderQueue);
  const pagedStatementLines = paged('statements', statementLines);
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)}>
          <TabsList className="h-auto flex-wrap justify-start bg-white border border-slate-200 rounded-full p-1 gap-0.5 mb-3">
            ...
            <TabsTrigger value="invoices" className="rounded-full px-4 py-1.5 text-xs font-semibold text-slate-500 data-[state=active]:bg-ink data-[state=active]:text-white data-[state=active]:shadow-none">
              Invoices
              <span className="ml-1.5 text-[10px] font-bold opacity-70">{loading ? '' : invoices.length}</span>
            </TabsTrigger>
            <TabsTrigger value="reminders" className="rounded-full px-4 py-1.5 text-xs font-semibold text-slate-500 data-[state=active]:bg-ink data-[state=active]:text-white data-[state=active]:shadow-none">
              Reminders
              <span className="ml-1.5 text-[10px] font-bold opacity-70">{loading ? '' : reminderQueue.length}</span>
            </TabsTrigger>
            <TabsTrigger value="bank" className="rounded-full px-4 py-1.5 text-xs font-semibold text-slate-500 data-[state=active]:bg-ink data-[state=active]:text-white data-[state=active]:shadow-none">
              Bank matching
              <span className="ml-1.5 text-[10px] font-bold opacity-70">{loading ? '' : statementLines.length}</span>
            </TabsTrigger>
            <TabsTrigger value="medical" className="rounded-full px-4 py-1.5 text-xs font-semibold text-slate-500 data-[state=active]:bg-ink data-[state=active]:text-white data-[state=active]:shadow-none">
              Medical aid
              <span className="ml-1.5 text-[10px] font-bold opacity-70">{loading ? '' : claims.length + authorizations.length}</span>
            </TabsTrigger>
          </TabsList>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: 'Total Revenue', value: loading ? '-' : formatZAR(totalRevenue), note: 'All invoices', gradient: 'from-navy-800 to-ink' },
            { label: 'Paid Invoices', value: loading ? '-' : formatZAR(paidInvoices), note: `${invoices.filter((inv) => inv.status === INVOICE_STATUS.PAID).length} paid`, gradient: 'from-teal to-[#0b6f71]' },
            { label: 'Outstanding Debt', value: loading ? '-' : formatZAR(debtBuckets.totalOutstanding), note: loading ? '-' : `${outstandingInvoices} outstanding`, gradient: 'from-[#9f2f2f] to-[#6f1d1d]' },
          ].map((card) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
          {BUCKETS.map(({ key, label }, i) => {
            const gradients = ['from-[#5b6b7f] to-[#3b4653]','from-navy-800 to-ink','from-[#b8742e] to-[#8f5a22]','from-[#a8552a] to-[#7d3d1d]','from-[#9f2f2f] to-[#6f1d1d]','from-[#6f1d1d] to-[#3d0f0f]'];
            return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { label: 'Medical Aid Claims', value: loading ? '-' : String(claims.length), note: `${submittedClaims} pending, ${approvedClaims} approved`, gradient: 'from-[#3f4c7a] to-[#2c365c]' },
            { label: 'Claimed Value', value: loading ? '-' : formatZAR(claims.reduce((sum, claim) => sum + Number(claim.amount_claimed || 0), 0)), note: 'All submitted claims', gradient: 'from-teal to-[#0b6f71]' },
          ].map((card) => (
            <div key={card.label} className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${card.gradient} p-5 text-white shadow-md`}>
              <p className="text-2xl font-bold leading-none mb-1">{card.value}</p>
              <p className="text-xs font-semibold opacity-75">{card.label}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { label: 'Pending Authorizations', value: loading ? '-' : String(pendingAuthorizations), note: `${approvedAuthorizations} approved`, gradient: 'from-[#b8742e] to-[#8f5a22]' },
            { label: 'Authorization Value', value: loading ? '-' : formatZAR(totalAuthorized), note: loading ? '-' : `${formatZAR(authShortfall)} shortfall`, gradient: 'from-teal to-[#0b6f71]' },
          ].map((card) => (
            <div key={card.label} className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${card.gradient} p-5 text-white shadow-md`}>
              <p className="text-2xl font-bold leading-none mb-1">{card.value}</p>
              <p className="text-xs font-semibold opacity-75">{card.label}</p>
        ...
        <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleSpecialSubmit}>
              ...
              <Button type="submit" disabled={Boolean(submitting)} className="bg-navy-800 hover:bg-ink border-0 shadow-md">{submitting === 'create_special_invoice' ? 'Creating...' : 'Create Special Invoice'}</Button>
            </form>
          </CardContent>
        </Card>
        <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
        ...
        <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={handleBankSubmit}>
              ...
              <Button type="submit" disabled={Boolean(submitting)} className="bg-navy-800 hover:bg-ink border-0 shadow-md">{submitting === 'import_statement_line' ? 'Importing...' : 'Import and Match'}</Button>
            </form>
          </CardContent>
        </Card>
        <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
