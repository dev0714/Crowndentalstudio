...
- [ ] **Step 1: Add the dashboard summary helper**
...
- [ ] **Step 2: Add the CSV register export helper**
...
- [ ] **Step 3: Add the risk signal helper**
...
- [ ] **Step 2: Load source data from CRM tables**

Use `supabaseServer` and the authenticated session to load:
- patients
- appointments
- leads
- lab cases
...
Each register row should include:
- `id`
- `label`
- `patient_id`
- `patient_name`
...
- stale recall follow-ups
- cases with no workflow progress

- [ ] **Step 6: Verify the route exports the expected groups**
...
- top summary cards
- role-aware dashboard panels
- register sections with filters
- patient audit timeline
- risk panel

- [ ] **Step 2: Load the payload from the API**
...
Each register section should support:
- filtering by type/status
- exporting filtered rows to CSV

- [ ] **Step 4: Add navigation links**

Add `/operations` to the sidebar and portal navigation so it is reachable from existing layouts.
...
The patient detail page should show a compact audit timeline backed by the operations API, including calls, bookings, consents, documents, invoices, claims, and lab events.

- [ ] **Step 2: Add compact risk summaries to the existing dashboards**

Each current dashboard page should show the relevant risk counts:
- dashboard: overall risk summary
- accounts: finance risks
- lab: overdue lab risks
- compliance: consent gaps

- [ ] **Step 3: Reuse the operations API**

These pages should fetch from `/api/crm/operations` rather than reconstructing the same logic locally.
...
Expected:
- build succeeds after the integrations
